...
:checkadb
.\tool\adb devices -l | findstr "Realme RMX" > nul
 if errorlevel 1 (	
    echo.
	echo ADB:
...
:pbrp 
echo.
.\tool\fastboot --disable-verification flash vbmeta .\tool\R1\vbmeta.img
echo.
echo flash Vbmeta Done.
echo.
.\tool\fastboot flash recovery .\tool\R1\recovery.img
echo.
